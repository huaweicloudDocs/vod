...
[示例1：媒资上传（20M以下）](示例1-媒资上传（20M以下）.md)已介绍了如何将小于20M大小的音视频文件上传到点播服务。若您需要上传的音视频文件的大小大于20M，您可也以调用VOD API进行分段上传。
...
6.  <a name="li1027416251210"></a>调用OBS的[上传段](https://support.huaweicloud.com/api-obs/obs_04_0099.html)接口，上传视频文件的第一个分段。
    ...
    2.  请求消息头中增加“Content-Type“，值设置为“application/octet-stream“。若[步骤5.a](#li146351927115017)设置了“content\_md5“参数，则请求消息头中必须增加“Content-MD5“，设置为对应分段的MD5值，否则将会导致上传失败。
...
11. <a name="li26881244162214"></a>调用OBS的[合并段](https://support.huaweicloud.com/api-obs/obs_04_0102.html)接口，将上传的分段合并成一个视频文件。
    ...
    4.  请求成功，返回合并段的相关信息。
        ...
        ```
        <?xml version="1.0" encoding="UTF-8" standalone="yes"?>
        <CompleteMultipartUploadResult xmlns="http://obs.myhwclouds.com/doc/2015-06-30/">
