...
若上传的单媒资文件大小小于20M，可以直接用PUT方法对该接口返回的地址进行上传。具体使用方法请参考[示例1：媒资上传（20M以下）](示例1-媒资上传（20M以下）.md)。

若上传的单个媒资大小大于20M，需要进行分段上传，该接口的具体使用方法请参考[示例2：媒资分段上传（20M以上）](示例2-媒资分段上传（20M以上）.md)。
